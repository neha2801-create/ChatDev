...
class Circle(pygame.sprite.Sprite):
    def __init__(self, screen_width, screen_height):
        super().__init__()
        self.radius = secrets.SystemRandom().randint(10, 30)
        ...
        side = secrets.SystemRandom().randint(0, 3)
        if side == 0:  # Top edge
            self.rect.centerx = secrets.SystemRandom().randint(0, screen_width)
            self.rect.centery = -self.radius
        elif side == 1:  # Right edge
            self.rect.centerx = screen_width + self.radius
            self.rect.centery = secrets.SystemRandom().randint(0, screen_height)
        ...
        self.velocity_x = secrets.SystemRandom().randint(-5, 5)
        self.velocity_y = secrets.SystemRandom().randint(-5, 5)
